/*
	CSC 415 HW#3- ASCII Character Count
	Parker Gray
*/
...
char childArray[ASCIICHARNUM]; //array for child threads to store ascii character counts
int parentArray[NUMTHREADS][ASCIICHARNUM]; //array for parent to hold results of childArray. 
...
void *charCounter(void *param) //the function that child threads will run when they are created.
{
    ...
    char currentCharacter = buf[locationInFile]; //holds the current character at location in character buffer file was loaded into.
    ...
    for (int i=0; i < childStruct->childPartitionSize && locationInFile < BUFSIZE; i++){ //for that continues until i reaches the end of the partition or the end of the buffer.
        currentCharacter = childArray[locationInFile++];  //get currentCharacter to be the childArray's location in the file.
        parentArray[childStruct->tid][currentCharacter]++; //tally up the [row][column] associated with the currently running child thread and the column of the character read.
    }
    pthread_exit(0);
}
...
int main(int argc, char *argv[])
{
    ...
    int partition = fileLength / NUMTHREADS; //create a partition by dividing the fileLength by 8.
    ...
    for (int j=0; j< ASCIICHARNUM; j++){ //columns
	sumChar = 0;
        for (int k = 0; k < NUMTHREADS; k++){ //rows
            sumChar += parentArray[k][j]; //sum is equal to the accumulation of ASCII char 0, 1, 2, 3 etc in childThreads 0 through 7.
        }
