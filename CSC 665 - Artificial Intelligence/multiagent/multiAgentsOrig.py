...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
		...
		closestDistance = 1e6
		for i in xrange(foodCount): # generate distances for closest food 
			distance = manhattanDistance(foodPos[i],newPos) + foodCount*100
			if distance < closestDistance: # if calculated distance is closer than the current closest distance
				closestDistance = distance # set that distance to be the closest distance
				closestFood = foodPos # closest food becomes the food that corresponds to closest distance
		if foodCount == 0 :
			closestDistance = 0
		score = -closestDistance

		for i in xrange(len(newGhostStates)): # generate distances for ghosts 
			ghostPos = successorGameState.getGhostPosition(i+1)
			if manhattanDistance(newPos,ghostPos)<=1 : #if the next position is the same position as a ghost position, don't go there
				score -= 1e6 
